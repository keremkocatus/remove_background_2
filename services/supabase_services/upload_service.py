...
async def upload_image(user_id: str, image_file: UploadFile, category: str) -> tuple[str, str] | None:
    try:
        ...
        await supabase.storage.from_(WARDROBE_BUCKET_NAME).upload(
            file=image_data,
            path=storage_path,
            file_options={"cache-control": "3600", "upsert": "false"},
        )

        public_url_response = await supabase.storage.from_(WARDROBE_BUCKET_NAME).get_public_url(storage_path)
...
async def upload_background_removed_image(processed_image: bytes, job_id: str, job: dict[str, str]) -> str | None:
    try:
        ...
        await supabase.from_(WARDROBE_BUCKET_NAME).update({
...
async def upload_image_edit(user_id: str, image: UploadFile):
    try:
        supabase = await get_supabase_client()
        image_data = await image.read()
        bucket_id = str(uuid.uuid4())
        storage_path = build_storage_path(user_id, bucket_id, "unedited_image.jpg")

        await supabase.storage.from_(EDIT_BUCKET_NAME).upload(
            file=image_data,
            path=storage_path,
            file_options={"cache-control": "3600", "upsert": "false"},
        )

        public_url_response = await supabase.storage.from_(EDIT_BUCKET_NAME).get_public_url(storage_path)
        public_url = extract_public_url(public_url_response)
...
async def upload_edited_image(processed_image: bytes, job: dict[str, str]) -> str | None:
    try:
        supabase = await get_supabase_client()
        storage_path = build_storage_path(job["user_id"], job["bucket_id"], "edited.png")
        ...
        await supabase.from_(EDIT_TABLE_NAME).update({
        ...
        print(f"Error in upload_edited_image: {error}")
        return None
    
async def upload_image_review(user_id: str, image: UploadFile):
    try:
        supabase = await get_supabase_client()
        image_data = await image.read()
        bucket_id = str(uuid.uuid4())
        storage_path = build_storage_path(user_id, bucket_id, "review_image.jpg")

        await supabase.storage.from_(REVIEW_BUCKET).upload(
            file=image_data,
            path=storage_path,
            file_options={"cache-control": "3600", "upsert": "false"},
        )

        public_url_response = await supabase.storage.from_(REVIEW_BUCKET).get_public_url(storage_path)
        public_url = extract_public_url(public_url_response)
